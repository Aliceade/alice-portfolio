            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 32, marginTop: 40, paddingTop: 32, borderTop: "1px solid rgba(255,247,232,0.1)" }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {p.stack.map((s) => (
                    <span key={s} style={{ padding: "3px 10px", borderRadius: 100, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, border: "1px solid rgba(255,187,166,0.3)", color: "var(--coral)" }}>
                      {s}
                    </span>
                  ))}
                </div>
        ...
        <section style={{ background: "var(--white)", padding: "64px 0" }}>
          <div className="wrap">
            <div className="gallery-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {/* Image 1 — full width */}
              <div style={{ gridColumn: "1 / -1", borderRadius: 14, overflow: "hidden", aspectRatio: "16/8", background: `linear-gradient(135deg, ${p.accentColor}35, ${p.accentColor}15)`, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <img src={`/portfolio/${p.slug}/1.jpg`} alt={`${p.name} screenshot 1`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, opacity: 0.3, letterSpacing: "0.1em", position: "relative", zIndex: 1 }}>{/*/portfolio/{p.slug}/1.jpg */}</span>
              </div>
              {/* Images 2 & 3 — side by side */}
              {["2", "3"].map((n) => (
                <div key={n} style={{ borderRadius: 14, overflow: "hidden", aspectRatio: "4/3", background: `linear-gradient(135deg, ${p.accentColor}25, ${p.accentColor}10)`, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <img src={`/portfolio/${p.slug}/${n}.jpg`} alt={`${p.name} screenshot ${n}`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} />
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, opacity: 0.3, letterSpacing: "0.1em", position: "relative", zIndex: 1 }}>{/*/portfolio/{p.slug}/{n}.jpg */}</span>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {p.results.map((r, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "16px 20px", background: "var(--white)", borderRadius: 10, border: "1px solid var(--border)" }}>
                        <span style={{ width: 22, height: 22, borderRadius: "50%", background: "var(--cv)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0, marginTop: 1 }}>{i + 1}</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 28, position: "sticky", top: 100 }}>
                ...
                <div style={{ background: "var(--forest)", borderRadius: 14, padding: 28 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {p.stack.map((s) => (
                      <span key={s} style={{ padding: "5px 12px", borderRadius: 100, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, border: "1px solid rgba(255,187,166,0.25)", color: "var(--coral)" }}>
                        {s}
                      </span>
                    ))}
                  </div>
          ...
          <div className="wrap" style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
            {prev ? (
              <Link href={`/portfolio/${prev.slug}`} style={{ textDecoration: "none", display: "flex", flexDirection: "column", gap: 4 }}>
                ...
                <span className="f-serif" style={{ fontSize: 18, color: "var(--forest)" }}>
                  {prev.name}
                </span>
              </Link>
            ) : (
